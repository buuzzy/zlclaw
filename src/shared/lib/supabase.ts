 ...
 *   - `pnpm tauri dev` → .env.development (或默认 fallback 到 prod)
 *   - `pnpm tauri build` → .env.production
 *
 * 为什么 anon key 可以公开：
 ...
 *   3. 把 dev project 的 URL / anon key 填到 .env.development
 *   4. prod project 的保持在 .env.production（即当前硬编码的老值作为兜底）
 */
