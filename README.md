...
> **模型选择对体验有显著影响**
> 记忆调取依赖模型在规划阶段主动调用 MCP 工具的能力。实测中：
> - **Claude Sonnet 4.6 / Opus 4.6 / GPT-5**：稳定按 AGENTS.md 触发条件调用 `search_memory`
> - **MiniMax-M2.7-highspeed**：规划阶段对外部工具的 follow-up 不稳定，记忆类问题可能直接回答而不查史
>
> 项目按"合理强模型"标准设计架构，不为单一模型加特殊路由兜底。如果记忆体验不佳，建议换用更强的对话模型；执行阶段（如有计划的多步任务）所有模型表现一致。
