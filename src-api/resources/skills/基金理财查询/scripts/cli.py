...
class FundDataAPIError(Exception):
    """API错误异常类"""
...
使用示例:
  ...
  python3 scripts/cli.py --query "这只基金的持仓明细" --page "1" --limit "20"
  python3 scripts/cli.py --query "基金风险评级" --api-key "your-key"
...
def query_fund_data(query: str, page: str, limit: str,
    ...
    Raises:
        FundDataAPIError: API调用失败时抛出
    """
    # 获取API密钥
    if not api_key:
        api_key = os.environ.get("IWENCAI_API_KEY", "")
    if not api_key:
        raise FundDataAPIError("API密钥未设置，请通过参数或环境变量IWENCAI_API_KEY指定")
    ...
    try:
        ...
        with urllib.request.urlopen(request, timeout=30) as response:
            ...
            if isinstance(result, dict):
                ...
                if status_code != 0:
                    ...
                    raise FundDataAPIError(f"API返回错误: {status_msg}")
        ...
        raise FundDataAPIError(
            f"HTTP错误 {e.code}: {e.reason}",
            status_code=e.code,
            response=error_body
        )
    except urllib.error.URLError as e:
        raise FundDataAPIError(f"网络错误: {e.reason}")
    except json.JSONDecodeError as e:
        raise FundDataAPIError(f"响应解析失败: {e}")
