---
name: 公司经营数据查询
description: 同花顺公司经营数据查询skill。查询主营业务构成、主要客户、供应商、参控股公司、股权投资、重大合同等经营相关数据，支持自然语言问句输入，返回相关经营数据结果。当用户询问主营业务构成、主要客户、供应商信息、参控股公司、股权投资、重大合同等经营数据查询问题时，必须使用此技能。
promptDescription: 同花顺经营数据：主营业务构成、主要客户、供应商、参控股公司
whenToUse: 主营业务,主要客户,供应商,参控股,子公司,股权投资,重大合同,经营数据
---
...
本技能提供公司经营数据查询能力，支持：
- 主营业务构成（产品/地区分布、收入占比等）
- 主要客户信息（客户名称、销售占比等）
- 供应商信息（供应商名称、采购占比等）
- 参控股公司（子公司、参股公司信息）
- 股权投资情况
- 重大合同信息
- 支持自然语言问句输入，返回相关经营数据结果
...
- **多次查询**：如果用户问题涉及多个维度（如"主营业务构成和主要客户"），需要拆分为多个标准 query 分别调用接口
...
# 查询主要客户
python3 scripts/cli.py --query "主要客户"

# 查询参控股公司
python3 scripts/cli.py --query "参控股公司"

# 翻页查询
python3 scripts/cli.py --query "主营业务构成" --page 2 --limit 20
```
