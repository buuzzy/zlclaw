---
name: 行业数据查询
description: 同花顺行业数据查询skill。查询行业估值、财务、盈利、行情、板块排名等数据，支持自然语言问句输入，返回相关行业数据结果。当用户询问行业数据、行业估值、行业排名、行业财务、行业盈利、行业行情、板块排名等行业相关问题时，必须使用此技能。
promptDescription: 同花顺行业：估值、财务、盈利、行情、板块排名等行业数据
