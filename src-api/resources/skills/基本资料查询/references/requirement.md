...
|------|------|
| **Skill 名称** | 基本资料查询 |
| **描述** | 基本资料数据查询skill。查询全品类标的（股票、指数、基金、期货、期权、转债、债券、理财、保险等）的基础信息、发行主体、机构资料、费率、上市地点、上市日期等静态信息，支持自然语言问句输入，返回相关基本资料数据结果 |
...
- API 调用失败时，给出友好的错误提示
- 无数据返回时，告知用户"暂未查到相关数据，请稍后重试或更换关键词"
...
```bash
# 直接查询（默认分页10条）
python scripts/cli.py --query "贵州茅台上市日期？"
...
---
name: 基本资料查询
description: 基本资料数据查询skill。查询全品类标的（股票、指数、基金、期货、期权、转债、债券、理财、保险等）的基础信息、发行主体、机构资料、费率、上市地点、上市日期等静态信息，支持自然语言问句输入，返回相关基本资料数据结果。
---
```
