...
| 港股 | `hk` | `hk00700`（腾讯控股） |
| 美股个股 | `us` | `usNVDA`（英伟达）/ `usAAPL`（苹果）/ `usTSLA`（特斯拉）|
| ETF | `sh/sz` | `sh510300`（沪深300ETF） |

⚠️ **美股个股**代码格式：`us` 小写 + **股票符号大写**（NVDA/AAPL/TSLA…）。
不接受 `nvda` / `us_nvda` / `NVDA` / `us.nvda` 等其他写法。

**港股 / 海外指数代码（特殊格式，必须严格匹配）**：

| 指数 | 代码 | 备注 |
|------|------|------|
| 恒生指数 | `hkHSI` | 注意大小写：前缀 `hk` 小写 + `HSI` 大写 |
| 恒生科技指数 | `hkHSTECH` | |
| 恒生中国企业指数（国企指数）| `hkHSCEI` | |
| 纳斯达克综合指数 | `usIXIC` | `us` 小写 + `IXIC` 大写 |
| 纳斯达克 100 | `usNDX` | |
| 标普 500 | `usSPX` | |
| 道琼斯工业指数 | `usDJI` | |

⚠️ 海外指数**只接受**上面的代码格式，不接受 `HSI` / `hk.HSI` / `^HSI` / `hk800000` 等其他写法。
查不到时不要反复尝试不同变体，直接换上面的标准代码。
