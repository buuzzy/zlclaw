...
export function appendOrCreateConversation(
  ...
  // ── T7: channel-store 写入 ──
  traceLog('T7', 'appendOrCreateConversation 调用', {
    channel,
    hasExisting: !!existing,
    existingId: existing?.id || '(新建)',
    existingVersion: existing?.version || 0,
    existingMsgCount: existing?.messages.length || 0,
    userMsg: userMessage.slice(0, 40),
  });
  ...
  if (existing) {
    ...
    traceLog('T7-OK', '追加到已有对话', {
      id: existing.id,
      newVersion: existing.version,
      newMsgCount: existing.messages.length,
    });
  ...
  traceLog('T7-OK', '创建新对话', {
    id,
    channel,
    totalConvs: conversations.size,
  });
...
/** Delete a channel conversation by ID */
