...
import { createSession, runAgent, type AgentMessage, type ConversationMessage } from '@/shared/services/agent';
...
import { matchSlashCommand, executeSlashCommand } from './slash-commands';
...
export class ChannelManager {
  ...
  async handleIncomingMessage(
    ...
    // ── T4: 消息入队 ──
    traceLog('T4', '消息入队 enqueue', {
      channelKey,
      content: incoming.content.slice(0, 60),
      queueSize: this.channelQueues.has(channelKey) ? '有前序任务' : '队列空闲',
    });
  ...
  private async _processMessage(
    ...
    traceLog('T5', '_processMessage 开始', {
      content: incoming.content.slice(0, 60),
    ...
    const command = matchSlashCommand(incoming.content);
    if (command) {
    ...
    // ─── Typing indicator: add ⏳ reaction to acknowledge receipt ────
    const supportsReaction = !!(adapter.addReaction && adapter.removeReaction);
    let typingReactionId: string | null = null;
    if (supportsReaction && incoming.replyToMessageId) {
      typingReactionId = await adapter.addReaction!(incoming.replyToMessageId, 'HOURGLASS').catch(() => null);
    }
    ...
    try {
      ...
      // Pass image attachments from channel message as multimodal content
      const channelImages = incoming.images?.map(img => ({
        data: img.data,
      ...
      if (supportsReaction && typingReactionId && incoming.replyToMessageId) {
        await adapter.removeReaction!(incoming.replyToMessageId, typingReactionId).catch(() => {});
      ...
      // ─── Token-budget pruning: keep history within budget ─────────
      this.pruneHistory(channelSession.history);

      const channelKey = `${adapter.id}:${incoming.conversationId}`;
      const storeResult = appendOrCreateConversation(channelKey, incoming.content, assistantText);

      traceLog('T6', '对话存储完成', {
        convId: storeResult.id,
        version: storeResult.version,
        msgCount: storeResult.messages.length,
        channelKey,
        userMsg: incoming.content.slice(0, 40),
        assistantMsg: assistantText.slice(0, 40),
      });
  ...
  private async handleSlashCommand(
    ...
    // Channel-specific side effects for /new and /reset
    if (command === 'new' || command === 'reset') {
      resetChannelSession(channelKey);
      const session = this.sessions.get(channelKey);
      if (session) session.history = [];
      // Clear compaction store so stale summaries don't leak into new conversations
      try {
        const { deleteCompaction } = require('@/shared/context/compaction-store');
        deleteCompaction(channelKey);
      } catch { /* ignore */ }
    }

    // Delegate to shared slash command handler with channel context
    const session = this.sessions.get(channelKey);
    const modelCfg = this.resolveModelConfig();
    let replyText = await executeSlashCommand({ name: command }, {
      taskId: channelKey,
      modelConfig: modelCfg,
      conversation: session?.history,
    });

    // Append channel-specific info for /status
    if (command === 'status') {
      replyText += `\n🔗 渠道：${channelKey}`;
    }
  ...
  /** Max tokens for in-memory channel history (default ~12K tokens ≈ 48K chars) */
  private static readonly HISTORY_TOKEN_BUDGET = 12000;

  /**
   * Prune history to stay within token budget.
   * Drops oldest message pairs (user+assistant) until under budget.
   * Context Assembler handles compaction/summarization separately.
   */
  private pruneHistory(history: ConversationMessage[]): void {
    const budget = ChannelManager.HISTORY_TOKEN_BUDGET;
    let totalTokens = 0;
    for (const msg of history) {
      totalTokens += Math.ceil(msg.content.length / 4);
    }

    // Drop oldest pairs (2 messages at a time) until within budget
    while (totalTokens > budget && history.length > 4) {
      const dropped1 = history.shift()!;
      const dropped2 = history.shift()!;
      totalTokens -= Math.ceil(dropped1.content.length / 4);
      totalTokens -= Math.ceil(dropped2.content.length / 4);
    }
  }
